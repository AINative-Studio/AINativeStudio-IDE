...
export const PastThreadsList = ({ className = '' }: {className?: string;}) => {
  ...
  if (!allThreads) {
    return <div key="error" className="ainative-p-1">{`Error accessing chat history.`}</div>;
  }
  ...
  return (
    <div className={`ainative-flex ainative-flex-col ainative-mb-2 ainative-gap-2 ainative-w-full ainative-text-nowrap ainative-text-ainative-fg-3 ainative-select-none ainative-relative ${className}`}>
			{displayThreads.length === 0 // this should never happen
      ? <></> :
      displayThreads.map((threadId, i) => {
        const pastThread = allThreads[threadId];
        if (!pastThread) {
          return <div key={i} className="ainative-p-1">{`Error accessing chat history.`}</div>;
        }
      ...
      <div
        className="ainative-text-ainative-fg-3 ainative-opacity-80 hover:ainative-opacity-100 hover:ainative-brightness-115 ainative-cursor-pointer ainative-p-1 ainative-text-xs"
...
const TrashButton = ({ threadId }: {threadId: string;}) => {
  ...
  <div className="ainative-flex ainative-flex-nowrap ainative-text-nowrap ainative-gap-1">
			<IconShell1
      Icon={X}
      className="ainative-size-[11px]"
...

{
  ...
  // data-tooltip-place='top'
  >
		<span className="ainative-opacity-60">{numMessages}</span>
		{` `}
  ...
  return <div
    ...
    onMouseLeave={() => setHoveredIdx(null)}>
		...
		<div className="ainative-flex ainative-items-center ainative-justify-between ainative-gap-1">
			<span className="ainative-flex ainative-items-center ainative-gap-2 ainative-min-w-0 ainative-overflow-hidden">
				{/* spinner */}
				{isRunning === 'LLM' || isRunning === 'tool' || isRunning === 'idle' ? <LoaderCircle className="ainative-animate-spin ainative-bg-ainative-stroke-1 ainative-flex-shrink-0 ainative-flex-grow-0" size={14} /> :

        isRunning === 'awaiting_user' ? <MessageCircleQuestion className="ainative-bg-ainative-stroke-1 ainative-flex-shrink-0 ainative-flex-grow-0" size={14} /> :
        ...
        data-tooltip-place='top'>
          {firstMsg}</span>
			...
			<div className="ainative-flex ainative-items-center ainative-gap-x-1 ainative-opacity-60">
				{idx === hoveredIdx ?
        <>
						{/* trash icon */}
