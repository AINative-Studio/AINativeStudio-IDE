...
import { ChatMode, displayInfoOfProviderName, FeatureName, isFeatureNameDisabled } from '../../../../../../../workbench/contrib/ainative/common/ainativeSettingsTypes.js';
import { ICommandService } from '../../../../../../../platform/commands/common/commands.js';
import { WarningBox } from '../ainative-settings-tsx/WarningBox.js';
...
import { ToolApprovalTypeSwitch } from '../ainative-settings-tsx/Settings.js';
...
const ReasoningOptionSlider = ({ featureName }: {featureName: FeatureName;}) => {
  ...
  if (canTurnOffReasoning && !reasoningBudgetSlider) {// if it's just a on/off toggle without a power slider
    return <div className="ainative-flex ainative-items-center ainative-gap-x-2">
			<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none ainative-inline-block ainative-w-10 ainative-pr-1">Thinking</span>
			<AINativeSwitch
        size='xxs'
    ...
    return <div className="ainative-flex ainative-items-center ainative-gap-x-2">
        ...
        onChange={(newVal) => {
          ...
          voidSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningBudget: newVal });
			...
			<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{isReasoningEnabled ? `${value} tokens` : 'Thinking disabled'}</span>
		</div>;
  }
    ...
    return <div className="ainative-flex ainative-items-center ainative-gap-x-2">
        ...
        onChange={(newVal) => {
          ...
          voidSettingsService.setOptionsOfModelSelection(featureName, modelSelection.providerName, modelSelection.modelName, { reasoningEnabled: !isOff, reasoningEffort: values[newVal] ?? undefined });
			...
			<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{isReasoningEnabled ? `${currentEffortCapitalized}` : 'Thinking disabled'}</span>
		</div>;
  }
...
export const AINativeChatArea: React.FC<AINativeChatAreaProps> = ({
  ...
  return (
    <div
      ref={divRef}
      className={` ainative-gap-x-1 ainative-flex ainative-flex-col ainative-p-2 ainative-relative ainative-input ainative-text-left ainative-shrink-0 ainative-rounded-md ainative-bg-ainative-bg-1 ainative-transition-all ainative-duration-200 ainative-border ainative-border-ainative-border-3 focus-within:ainative-border-ainative-border-1 hover:ainative-border-ainative-border-1 ainative-max-h-[80vh] ainative-overflow-y-auto ${
      ...
      <SelectedFiles
        ...
        showProspectiveSelections={showProspectiveSelections} />
			...
			<div className="ainative-relative ainative-w-full">
				...
				{onClose &&
        <div className="ainative-absolute -ainative-top-1 -ainative-right-1 ainative-cursor-pointer ainative-z-1">
						<IconX
            size={12}
            className="ainative-stroke-[2] ainative-opacity-80 ainative-text-ainative-fg-3 hover:ainative-brightness-95"
            onClick={onClose} />
			...
			{/* Bottom row */}
			<div className="ainative-flex ainative-flex-row ainative-justify-between ainative-items-end ainative-gap-1">
				{showModelDropdown &&
        <div className="ainative-flex ainative-flex-col ainative-gap-y-1">
						<ReasoningOptionSlider featureName={featureName} />

						<div className="ainative-flex ainative-items-center ainative-flex-wrap ainative-gap-x-2 ainative-gap-y-1 ainative-text-nowrap ">
							{featureName === 'Chat' && <ChatModeDropdown className="ainative-text-xs ainative-text-ainative-fg-3 ainative-bg-ainative-bg-1 ainative-border ainative-border-ainative-border-2 ainative-rounded ainative-py-0.5 ainative-px-1" />}
							<ModelDropdown featureName={featureName} className="ainative-text-xs ainative-text-ainative-fg-3 ainative-bg-ainative-bg-1 ainative-rounded" />
						</div>
					</div>
        }
...
export const ButtonSubmit = ({ className, disabled, ...props }: ButtonProps & Required<Pick<ButtonProps, 'disabled'>>) => {
  ...
  return <button
    type='button'
    className={`ainative-rounded-full ainative-flex-shrink-0 ainative-flex-grow-0 ainative-flex ainative-items-center ainative-justify-center ${
    disabled ? "ainative-bg-vscode-disabled-fg ainative-cursor-default" : "ainative-bg-white ainative-cursor-pointer"} ${
    className} `}
    ...
    {...props}>
		...
		<IconArrowUp size={DEFAULT_BUTTON_SIZE} className="ainative-stroke-[2] ainative-p-[2px]" />
	</button>;
};

export const ButtonStop = ({ className, ...props }: ButtonHTMLAttributes<HTMLButtonElement>) => {
  return <button
    className={`ainative-rounded-full ainative-flex-shrink-0 ainative-flex-grow-0 ainative-cursor-pointer ainative-flex ainative-items-center ainative-justify-center ainative-bg-white ${
    ...
    {...props}>
		...
		<IconSquare size={DEFAULT_BUTTON_SIZE} className="ainative-stroke-[3] ainative-p-[7px]" />
	</button>;
};
...
}: {type: 'past';selections: StagingSelectionItem[];setSelections?: undefined;showProspectiveSelections?: undefined;messageIdx: number;} | {type: 'staging';selections: StagingSelectionItem[];setSelections: ((newSelections: StagingSelectionItem[]) => void);showProspectiveSelections?: boolean;messageIdx?: number;}) =>
{
  ...
  const accessor = useAccessor();
  const commandService = accessor.get('ICommandService');
  const modelReferenceService = accessor.get('IAINativeModelService');
  ...
  return (
    <div className="ainative-flex ainative-items-center ainative-flex-wrap ainative-text-left ainative-relative ainative-gap-x-0.5 ainative-gap-y-1 ainative-pb-0.5">
        ...
        key={thisKey}
        className={`ainative-flex ainative-flex-col ainative-space-y-[1px]`}>

					{/* tooltip for file path */}
					<span className="ainative-truncate ainative-overflow-hidden ainative-text-ellipsis"
          data-tooltip-id='ainative-tooltip'
          ...
          data-tooltip-delay-show={3000}>
						...
						{/* summarybox */}
						<div
              className={` ainative-flex ainative-items-center ainative-gap-1 ainative-relative ainative-px-1 ainative-w-fit ainative-h-fit ainative-select-none ainative-text-xs ainative-text-nowrap ainative-border ainative-rounded-sm ${
              ...
              isThisSelectionProspective ? "ainative-bg-ainative-bg-1 ainative-text-ainative-fg-3 ainative-opacity-80" : "ainative-bg-ainative-bg-1 hover:ainative-brightness-95 ainative-text-ainative-fg-1"} ${
              isThisSelectionProspective ? "ainative-border-ainative-border-2" : "ainative-border-ainative-border-1"} hover:ainative-border-ainative-border-1 ainative-transition-all ainative-duration-150 `}
              ...
              <span className={`ainative-text-[8px] ainative-'void-opacity-60 ainative-text-ainative-fg-4`}>
									{`(Current File)`}
								</span> :
              null
              }
              ...
              className="ainative-cursor-pointer ainative-z-1 ainative-self-stretch ainative-flex ainative-items-center ainative-justify-center"
              onClick={(e) => {
...
const ToolHeaderWrapper = ({
  ...
  const desc1HTML = <span
    className={`ainative-text-ainative-fg-4 ainative-text-xs ainative-italic ainative-truncate ainative-ml-2 ${
    isDesc1Clickable ? "ainative-cursor-pointer hover:ainative-brightness-125 ainative-transition-all ainative-duration-150" : ""} `}
  ...
  return <div className="">
		<div className={`ainative-w-full ainative-border ainative-border-ainative-border-3 ainative-rounded ainative-px-2 ainative-py-1 ainative-bg-ainative-bg-3 ainative-overflow-hidden ${className}`}>
			{/* header */}
			<div className={`ainative-select-none ainative-flex ainative-items-center ainative-min-h-[24px]`}>
				<div className={`ainative-flex ainative-items-center ainative-w-full ainative-gap-x-2 ainative-overflow-hidden ainative-justify-between ${isRejected ? "ainative-line-through" : ""}`}>
					{/* left */}
					<div // container for if desc1 is clickable
          className="ainative-ml-1 ainative-flex ainative-items-center ainative-overflow-hidden">

						{/* title eg "> Edited File" */}
						<div className={` ainative-flex ainative-items-center ainative-min-w-0 ainative-overflow-hidden ainative-grow ${

            isClickable ? "ainative-cursor-pointer hover:ainative-brightness-125 ainative-transition-all ainative-duration-150" : ""} `}
            ...
            onClick={() => {
              ...
              if (onClick) {onClick();}
                ...
                className={` ainative-text-ainative-fg-3 ainative-mr-0.5 ainative-h-4 ainative-w-4 ainative-flex-shrink-0 ainative-transition-transform ainative-duration-100 ainative-ease-[cubic-bezier(0.4,0,0.2,1)] ${

                isExpanded ? "ainative-rotate-90" : ""} `} />

              }
							<span className="ainative-text-ainative-fg-3 ainative-flex-shrink-0">{title}</span>
					...
					{/* right */}
					<div className="ainative-flex ainative-items-center ainative-gap-x-2 ainative-flex-shrink-0">

						{info && <CircleEllipsis
              className="ainative-ml-2 ainative-text-ainative-fg-4 ainative-opacity-60 ainative-flex-shrink-0"
              size={14}
              data-tooltip-id='ainative-tooltip'
              ...
              className="ainative-text-ainative-warning ainative-opacity-90 ainative-flex-shrink-0"
              size={14}
              data-tooltip-id='ainative-tooltip'
              ...
              data-tooltip-place='top' />
						...
						{isRejected && <Ban
              className="ainative-text-ainative-fg-4 ainative-opacity-90 ainative-flex-shrink-0"
              size={14}
              data-tooltip-id='ainative-tooltip'
              data-tooltip-content={'Canceled'}
              data-tooltip-place='top' />
            }
						{desc2 && <span className="ainative-text-ainative-fg-4 ainative-text-xs" onClick={desc2OnClick}>
							{desc2}
						</span>}
						{numResults !== undefined &&
            <span className="ainative-text-ainative-fg-4 ainative-text-xs ainative-ml-auto ainative-mr-1">
			...
			{/* children */}
			{<div
        className={`ainative-overflow-hidden ainative-transition-all ainative-duration-200 ainative-ease-in-out ${isExpanded ? "ainative-opacity-100 ainative-py-1" : "ainative-max-h-0 ainative-opacity-0"} ainative-text-ainative-fg-4 ainative-rounded-sm ainative-overflow-x-auto `}


        //    bg-black bg-opacity-10 border border-ainative-border-4 border-opacity-50
      >
				{children}
			</div>}
		</div>
...
const EditTool = ({ toolMessage, threadId, messageIdx, content }: Parameters<ResultWrapper<'edit_file' | 'rewrite_file'>>[0] & {content: string;}) => {
  ...
  if (toolMessage.type === 'running_now' || toolMessage.type === 'tool_request') {
    componentParams.children = <ToolChildrenWrapper className="ainative-bg-ainative-bg-3">
			<EditToolChildren
        uri={params.uri}
        code={content}
        type={editToolType} />
  ...
  if (toolMessage.type === 'success' || toolMessage.type === 'rejected' || toolMessage.type === 'tool_error') {
    ...
    if (toolMessage.type === 'success' || toolMessage.type === 'rejected') {
      ...
      componentParams.bottomChildren = <BottomChildren title='Lint errors'>
        ...
        <div key={i} className="ainative-whitespace-nowrap">Lines {error.startLineNumber}-{error.endLineNumber}: {error.message}</div>
        )}
			</BottomChildren>;
    } else
  ...
  return (
    <div>
			<div className="ainative-w-full">
				{/* header */}
				<div
          className={`ainative-select-none ainative-flex ainative-items-center ainative-min-h-[24px] ${isDropdown ? "ainative-cursor-pointer" : ""}`}
          onClick={() => {
            if (isDropdown) {setIsOpen((v) => !v);}
          }}>
					...
					{isDropdown &&
          <ChevronRight
            className={`ainative-text-ainative-fg-3 ainative-mr-0.5 ainative-h-4 ainative-w-4 ainative-flex-shrink-0 ainative-transition-transform ainative-duration-100 ainative-ease-[cubic-bezier(0.4,0,0.2,1)] ${isOpen ? "ainative-rotate-90" : ""}`} />

          }
					<div className="ainative-flex ainative-items-center ainative-w-full ainative-overflow-hidden">
						<span className="ainative-text-ainative-fg-3">{title}</span>
					</div>
				</div>
				{/* children */}
				{<div
          className={`ainative-overflow-hidden ainative-transition-all ainative-duration-200 ainative-ease-in-out ${isOpen ? "ainative-opacity-100" : "ainative-max-h-0 ainative-opacity-0"} ainative-text-ainative-fg-4`}>
...
const UserMessageComponent = ({ chatMessage, messageIdx, isCheckpointGhost, currCheckpointIdx, _scrollToBottom }: {chatMessage: ChatMessage & {role: 'user';};messageIdx: number;currCheckpointIdx: number | undefined;isCheckpointGhost: boolean;_scrollToBottom: (() => void) | null;}) => {
  ...
  if (mode === 'display') {
    chatbubbleContents = <>
			...
			<span className="ainative-px-0.5">{chatMessage.displayContent}</span>
		</>;
  } else
  if (mode === 'edit') {
    ...
    chatbubbleContents = <AINativeChatArea
      featureName='Chat'
      onSubmit={onSubmit}
      onAbort={onAbort}
      isStreaming={false}
      ...
      setSelections={setStagingSelections}>
        ...
        enableAtToMention
        ref={setTextAreaRef}
        className="ainative-min-h-[81px] ainative-max-h-[500px] ainative-px-0.5"
  ...
  className={` ainative-relative ainative-ml-auto ${

  mode === 'edit' ? "ainative-w-full ainative-max-w-full" :
  mode === 'display' ? `ainative-self-end ainative-w-fit ainative-max-w-full ainative-whitespace-pre-wrap` : ""} ${


  isCheckpointGhost && !isMsgAfterCheckpoint ? "ainative-opacity-50 ainative-pointer-events-none" : ""} `}
  ...
  onMouseLeave={() => setIsHovered(false)}>
    ...
    className={` ainative-text-left ainative-rounded-lg ainative-max-w-full ${

    mode === 'edit' ? "" :
    mode === 'display' ? "ainative-p-2 ainative-flex ainative-flex-col ainative-bg-ainative-bg-1 ainative-text-ainative-fg-1 ainative-overflow-x-auto ainative-cursor-pointer" : ""} `}
    ...
    onClick={() => {if (mode === 'display') {onOpenEdit();}}}>
		...
		<div
      className="ainative-absolute -ainative-top-1 -ainative-right-1 ainative-translate-x-0 -ainative-translate-y-0 ainative-z-1"
      // data-tooltip-id='ainative-tooltip'
    ...
    >
			<EditSymbol
        size={18}
        className={` ainative-cursor-pointer ainative-p-[2px] ainative-bg-ainative-bg-1 ainative-border ainative-border-ainative-border-1 ainative-rounded-md ainative-transition-opacity ainative-duration-200 ainative-ease-in-out ${




        isHovered || isFocused && mode === 'edit' ? "ainative-opacity-100" : "ainative-opacity-0"} `}
...
const SmallProseWrapper = ({ children }: {children: React.ReactNode;}) => {
  return <div className=" ainative-text-ainative-fg-4 ainative-prose ainative-prose-sm ainative-break-words ainative-max-w-none ainative-leading-snug ainative-text-[13px] [&>:first-child]:!ainative-mt-0 [&>:last-child]:!ainative-mb-0 prose-h1:ainative-text-[14px] prose-h1:ainative-my-4 prose-h2:ainative-text-[13px] prose-h2:ainative-my-4 prose-h3:ainative-text-[13px] prose-h3:ainative-my-3 prose-h4:ainative-text-[13px] prose-h4:ainative-my-2 prose-p:ainative-my-2 prose-p:ainative-leading-snug prose-hr:ainative-my-2 prose-ul:ainative-my-2 prose-ul:ainative-pl-4 prose-ul:ainative-list-outside prose-ul:ainative-list-disc prose-ul:ainative-leading-snug prose-ol:ainative-my-2 prose-ol:ainative-pl-4 prose-ol:ainative-list-outside prose-ol:ainative-list-decimal prose-ol:ainative-leading-snug marker:ainative-text-inherit prose-blockquote:ainative-pl-2 prose-blockquote:ainative-my-2 prose-code:ainative-text-ainative-fg-3 prose-code:ainative-text-[12px] prose-code:before:ainative-content-none prose-code:after:ainative-content-none prose-pre:ainative-text-[12px] prose-pre:ainative-p-2 prose-pre:ainative-my-2 prose-table:ainative-text-[13px] ">
...
const ProseWrapper = ({ children }: {children: React.ReactNode;}) => {
  return <div className=" ainative-text-ainative-fg-2 ainative-prose ainative-prose-sm ainative-break-words prose-p:ainative-block prose-hr:ainative-my-4 prose-pre:ainative-my-2 marker:ainative-text-inherit prose-ol:ainative-list-outside prose-ol:ainative-list-decimal prose-ul:ainative-list-outside prose-ul:ainative-list-disc prose-li:ainative-my-0 prose-code:before:ainative-content-none prose-code:after:ainative-content-none prose-headings:ainative-prose-sm prose-headings:ainative-font-bold prose-p:ainative-leading-normal prose-ol:ainative-leading-normal prose-ul:ainative-leading-normal ainative-max-w-none ">
...
const AssistantMessageComponent = ({ chatMessage, isCheckpointGhost, isCommitted, messageIdx }: {chatMessage: ChatMessage & {role: 'assistant';};isCheckpointGhost: boolean;messageIdx: number;isCommitted: boolean;}) => {
  ...
  return <>
		{/* reasoning token */}
		{hasReasoning &&
    <div className={`${isCheckpointGhost ? "ainative-opacity-50" : ""}`}>
...
const ReasoningWrapper = ({ isDoneReasoning, isStreaming, children }: {isDoneReasoning: boolean;isStreaming: boolean;children: React.ReactNode;}) => {
  ...
  return <ToolHeaderWrapper title='Reasoning' desc1={isWriting ? <IconLoading /> : ''} isOpen={isOpen} onClick={() => setIsOpen((v) => !v)}>
		<ToolChildrenWrapper>
			<div className="!ainative-select-text ainative-cursor-auto">
				{children}
			</div>
		</ToolChildrenWrapper>
	</ToolHeaderWrapper>;
...
const loadingTitleWrapper = (item: React.ReactNode): React.ReactNode => {
  return <span className="ainative-flex ainative-items-center ainative-flex-nowrap">
		{item}
		<IconLoading className="ainative-w-3 ainative-text-sm" />
	</span>;
};
...
const ToolRequestAcceptRejectButtons = ({ toolName }: {toolName: ToolName;}) => {
  ...
  <button
    onClick={onAccept}
    className={` ainative-px-2 ainative-py-1 ainative-bg-[var(--vscode-button-background)] ainative-text-[var(--vscode-button-foreground)] hover:ainative-bg-[var(--vscode-button-hoverBackground)] ainative-rounded ainative-text-sm ainative-font-medium `}>
  ...
  <button
    onClick={onReject}
    className={` ainative-px-2 ainative-py-1 ainative-bg-[var(--vscode-button-secondaryBackground)] ainative-text-[var(--vscode-button-secondaryForeground)] hover:ainative-bg-[var(--vscode-button-secondaryHoverBackground)] ainative-rounded ainative-text-sm ainative-font-medium `}>
  ...
  const approvalToggle = approvalType ? <div key={approvalType} className="ainative-flex ainative-items-center ainative-ml-2 ainative-gap-x-1">
		<ToolApprovalTypeSwitch size='xs' approvalType={approvalType} desc={`Auto-approve ${approvalType}`} />
	</div> : null;

  return <div className="ainative-flex ainative-gap-2 ainative-mx-0.5 ainative-items-center">
		{approveButton}
		{cancelButton}
		{approvalToggle}
	</div>;
...
export const ToolChildrenWrapper = ({ children, className }: {children: React.ReactNode;className?: string;}) => {
  return <div className={`${className ? className : ""} ainative-cursor-default ainative-select-none`}>
		<div className="ainative-px-2 ainative-min-w-full ainative-overflow-hidden">
			{children}
		</div>
	</div>;
};
export const CodeChildren = ({ children, className }: {children: React.ReactNode;className?: string;}) => {
  return <div className={`${className ?? ''} ainative-p-1 ainative-rounded-sm ainative-overflow-auto ainative-text-sm`}>
		<div className="!ainative-select-text ainative-cursor-auto">
			{children}
		</div>
	</div>;
...
export const ListableToolItem = ({ name, onClick, isSmall, className, showDot }: {name: React.ReactNode;onClick?: () => void;isSmall?: boolean;className?: string;showDot?: boolean;}) => {
  return <div
    className={` ${
    onClick ? "hover:ainative-brightness-125 hover:ainative-cursor-pointer ainative-transition-all ainative-duration-200 " : ""} ainative-flex ainative-items-center ainative-flex-nowrap ainative-whitespace-nowrap ${

    className ? className : ""} `}

    onClick={onClick}>

		{showDot === false ? null : <div className="ainative-flex-shrink-0"><svg className="ainative-w-1 ainative-h-1 ainative-opacity-60 ainative-mr-1.5 ainative-fill-current" viewBox="0 0 100 40"><rect x="0" y="15" width="100" height="10" /></svg></div>}
		<div className={`${isSmall ? "ainative-italic ainative-text-ainative-fg-4 ainative-flex ainative-items-center" : ""}`}>{name}</div>
	</div>;
};
...
const EditToolChildren = ({ uri, code, type }: {uri: URI | undefined;code: string;type: 'diff' | 'rewrite';}) => {
  ...
  <AINativeDiffEditor uri={uri} searchReplaceBlocks={code} /> :
  <ChatMarkdownRender string={`\`\`\`\n${code}\n\`\`\``} codeURI={uri} chatMessageLocation={undefined} />;

  return <div className="!ainative-select-text ainative-cursor-auto">
		<SmallProseWrapper>
			{content}
		</SmallProseWrapper>
	</div>;
...
const LintErrorChildren = ({ lintErrors }: {lintErrors: LintErrorItem[];}) => {
  return <div className="ainative-text-xs ainative-text-ainative-fg-4 ainative-opacity-80 ainative-border-l-2 ainative-border-ainative-warning ainative-px-2 ainative-py-0.5 ainative-flex ainative-flex-col ainative-gap-0.5 ainative-overflow-x-auto ainative-whitespace-nowrap">
...
const BottomChildren = ({ children, title }: {children: React.ReactNode;title: string;}) => {
  ...
  return (
    <div className="ainative-w-full ainative-px-2 ainative-mt-0.5">
			<div
        className={`ainative-flex ainative-items-center ainative-cursor-pointer ainative-select-none ainative-transition-colors ainative-duration-150 ainative-pl-0 ainative-py-0.5 ainative-rounded ainative-group`}
        onClick={() => setIsOpen((o) => !o)}
        style={{ background: 'none' }}>

				<ChevronRight
          className={`ainative-mr-1 ainative-h-3 ainative-w-3 ainative-flex-shrink-0 ainative-transition-transform ainative-duration-100 ainative-text-ainative-fg-4 group-hover:ainative-text-ainative-fg-3 ${isOpen ? "ainative-rotate-90" : ""}`} />

				<span className="ainative-font-medium ainative-text-ainative-fg-4 group-hover:ainative-text-ainative-fg-3 ainative-text-xs">{title}</span>
			</div>
			<div
        className={`ainative-overflow-hidden ainative-transition-all ainative-duration-200 ainative-ease-in-out ${isOpen ? "ainative-opacity-100" : "ainative-max-h-0 ainative-opacity-0"} ainative-text-xs ainative-pl-4`}>

				<div className="ainative-overflow-x-auto ainative-text-ainative-fg-4 ainative-opacity-90 ainative-border-l-2 ainative-border-ainative-warning ainative-px-2 ainative-py-0.5">
					{children}
				</div>
			</div>
		</div>);
...
const EditToolHeaderButtons = ({ applyBoxId, uri, codeStr, toolName, threadId }: {threadId: string;applyBoxId: string;uri: URI;codeStr: string;toolName: 'edit_file' | 'rewrite_file';}) => {
  ...
  return <div className="ainative-flex ainative-items-center ainative-gap-1">
...
const InvalidTool = ({ toolName, message, mcpServerName }: {toolName: ToolName;message: string;mcpServerName: string | undefined;}) => {
  ...
  componentParams.children = <ToolChildrenWrapper>
		<CodeChildren className="ainative-bg-ainative-bg-3">
			{message}
		</CodeChildren>
	</ToolChildrenWrapper>;
  return <ToolHeaderWrapper {...componentParams} />;
...
const CommandTool = ({ toolMessage, type, threadId
  ...
  if (toolMessage.type === 'success') {
    ...
    componentParams.children = <ToolChildrenWrapper className="ainative-whitespace-pre ainative-text-nowrap ainative-overflow-auto ainative-text-sm">
			<div className="!ainative-select-text ainative-cursor-auto">
  ...
  if (toolMessage.type === 'running_now') {
    if (type === 'run_command')
    componentParams.children = <div ref={divRef} className="ainative-relative ainative-h-[300px] ainative-text-sm" />;
  } else
    ...
    resultWrapper: ({ toolMessage }) => {
      ...
      if (toolMessage.type === 'success') {
        ...
        <ToolChildrenWrapper>
          ...
          className="ainative-w-full ainative-overflow-auto"
          onClick={() => {
            ...
            // explorerService.select(child.uri, true);
						...
						{result.hasNextPage &&
          <ListableToolItem name={`Results truncated (${result.itemsRemaining} remaining).`} isSmall={true} className="ainative-w-full ainative-overflow-auto" />
          }
					</ToolChildrenWrapper>;
      } else
      if (toolMessage.type === 'tool_error') {
      ...
      if (toolMessage.type === 'success') {
        ...
        <ToolChildrenWrapper>
          ...
          onClick={() => {voidOpenFileFn(uri, accessor);}} />
						...
						{result.hasNextPage &&
          <ListableToolItem name={'Results truncated.'} isSmall={true} className="ainative-w-full ainative-overflow-auto" />
          }
  ...
  'search_in_file': {
    resultWrapper: ({ toolMessage }) => {
      ...
      if (toolMessage.type === 'success') {
        ...
        <ToolChildrenWrapper>
						<CodeChildren className="ainative-bg-ainative-bg-3">
							<pre className="ainative-font-mono ainative-whitespace-pre">
...
const Checkpoint = ({ message, threadId, messageIdx, isCheckpointGhost, threadIsRunning }: {message: CheckpointEntry;threadId: string;messageIdx: number;isCheckpointGhost: boolean;threadIsRunning: boolean;}) => {
  ...
  return <div
    className={`ainative-flex ainative-items-center ainative-justify-center ainative-px-2 `}>

		<div
      className={` ainative-text-xs ainative-text-ainative-fg-3 ainative-select-none ${



      isCheckpointGhost ? "ainative-opacity-50" : "ainative-opacity-100"} ${
      isDisabled ? "ainative-cursor-default" : "ainative-cursor-pointer"} `}
...
const _ChatBubble = ({ threadId, chatMessage, currCheckpointIdx, isCommitted, messageIdx, chatIsRunning, _scrollToBottom }: ChatBubbleProps) => {
  ...
  if (role === 'tool') {
    ...
    if (chatMessage.type === 'invalid_params') {
      return <div className={`${isCheckpointGhost ? "ainative-opacity-50" : ""}`}>
    ...
    return <>
      ...
      <div className={`${isCheckpointGhost ? "ainative-opacity-50 ainative-pointer-events-none" : ""}`}>
...
const CommandBarInChat = () => {
  ...
  const threadStatusHTML = <StatusIndicator className="ainative-mx-1" indicatorColor={threadStatus.color} title={threadStatus.title} />;
  ...
  className={`ainative-flex ainative-items-center ainative-gap-0.5 ${
  isFinishedMakingThreadChanges ? "" : "ainative-opacity-0 ainative-pointer-events-none"}`}>
  ...
  const fileDetailsContent = <div className="ainative-px-2 ainative-gap-1 ainative-w-full ainative-overflow-y-auto">
      ...
      const fileNameHTML = <div
        className="ainative-flex ainative-items-center ainative-gap-1.5 ainative-text-ainative-fg-3 hover:ainative-brightness-125 ainative-transition-all ainative-duration-200 ainative-cursor-pointer"
        onClick={() => voidOpenFileFn(uri, accessor)}>

				{/* <FileIcon size={14} className="text-ainative-fg-3" /> */}
				<span className="ainative-text-ainative-fg-3">{basename}</span>
			</div>;




      const detailsContent = <div className="ainative-flex ainative-px-4">
				<span className="ainative-text-ainative-fg-3 ainative-opacity-80">{numDiffs} diff{numDiffs !== 1 ? 's' : ''}</span>
			</div>;

      const acceptRejectButtons = <div
      // do this with opacity so that the height remains the same at all times
      className={`ainative-flex ainative-items-center ainative-gap-0.5 ${
      isFinishedMakingFileChanges ? "" : "ainative-opacity-0 ainative-pointer-events-none"} `}>
      ...
      const fileStatusHTML = <StatusIndicator className="ainative-mx-1" indicatorColor={fileStatus.color} title={fileStatus.title} />;

      return (
        // name, details
        <div key={i} className="ainative-flex ainative-justify-between ainative-items-center">
					<div className="ainative-flex ainative-items-center">
						{fileNameHTML}
						{detailsContent}
					</div>
					<div className="ainative-flex ainative-items-center ainative-gap-2">
  ...
  <button
    className={`ainative-flex ainative-items-center ainative-gap-1 ainative-rounded ${numFilesChanged === 0 ? "ainative-cursor-pointer" : "ainative-cursor-pointer hover:ainative-brightness-125 ainative-transition-all ainative-duration-200"}`}
    ...
    disabled={numFilesChanged === 0}>
			...
			<svg
      className="ainative-transition-transform ainative-duration-200 ainative-size-3.5"
      style={{
  ...
  return (
    <>
			{/* file details */}
			<div className="ainative-px-2">
				<div
          className={` ainative-select-none ainative-flex ainative-w-full ainative-rounded-t-lg ainative-bg-ainative-bg-3 ainative-text-ainative-fg-3 ainative-text-xs ainative-text-nowrap ainative-overflow-hidden ainative-transition-all ainative-duration-200 ainative-ease-in-out ${





          isFileDetailsOpened ? "ainative-max-h-24" : "ainative-max-h-0"} `}>
			...
			{/* main content */}
			<div
        className={` ainative-select-none ainative-flex ainative-w-full ainative-rounded-t-lg ainative-bg-ainative-bg-3 ainative-text-ainative-fg-3 ainative-text-xs ainative-text-nowrap ainative-border-t ainative-border-l ainative-border-r ainative-border-zinc-300/10 ainative-px-2 ainative-py-1 ainative-justify-between `}>
				...
				<div className="ainative-flex ainative-gap-2 ainative-items-center">
					{fileDetailsButton}
				</div>
				<div className="ainative-flex ainative-gap-2 ainative-items-center">
...
const EditToolSoFar = ({ toolCallSoFar }: {toolCallSoFar: RawToolCallObj;}) => {
  ...
  const desc1 = <span className="ainative-flex ainative-items-center">
		{uriDone ?
...
export const SidebarChat = () => {
  ...
  const messagesHTML = <ScrollToBottomContainer
    ...
    className={` ainative-flex ainative-flex-col ainative-px-4 ainative-py-4 ainative-space-y-4 ainative-w-full ainative-h-full ainative-overflow-x-hidden ainative-overflow-y-auto ${





    previousMessagesHTML.length === 0 && !displayContentSoFar ? "ainative-hidden" : ""} `}>
		...
		{isRunning === 'LLM' || isRunning === 'idle' && !toolIsGenerating ? <ProseWrapper>
			{<IconLoading className="ainative-opacity-50 ainative-text-sm" />}
		</ProseWrapper> : null}


		{/* error message */}
		{latestError === undefined ? null :
    <div className="ainative-px-2 ainative-my-1">
				<ErrorDisplay
        ...
        showDismiss={true} />
				...
				<WarningBox className="ainative-text-sm ainative-my-2 ainative-mx-4" onClick={() => {commandService.executeCommand(VOID_OPEN_SETTINGS_ACTION_ID);}} text='Open settings' />
			</div>
    }
  ...
  const inputChatArea = <AINativeChatArea
    featureName='Chat'
    onSubmit={() => onSubmit()}
    onAbort={onAbort}
    ...
    onClickAnywhere={() => {textAreaRef.current?.focus();}}>
		...
		<AINativeInputBox2
      enableAtToMention
      className={`ainative-min-h-[81px] ainative-px-0.5 ainative-py-0.5`}
  ...
  const initiallySuggestedPromptsHTML = <div className="ainative-flex ainative-flex-col ainative-gap-2 ainative-w-full ainative-text-nowrap ainative-text-ainative-fg-3 ainative-select-none">
		{[
    'Summarize my codebase',
    'How do types work in Rust?',
    'Create a .ainativerules file for me'].
    map((text, index) =>
    <div
      key={index}
      className="ainative-py-1 ainative-px-2 ainative-rounded ainative-text-sm ainative-bg-zinc-700/5 hover:ainative-bg-zinc-700/10 dark:ainative-bg-zinc-300/5 dark:hover:ainative-bg-zinc-300/10 ainative-cursor-pointer ainative-opacity-80 hover:ainative-opacity-100"
  ...
  const threadPageInput = <div key={'input' + chatThreadsState.currentThreadId}>
		<div className="ainative-px-4">
			<CommandBarInChat />
		</div>
		<div className="ainative-px-2 ainative-pb-2">
			{inputChatArea}
		</div>
	</div>;

  const landingPageInput = <div>
		<div className="ainative-pt-8">
			{inputChatArea}
		</div>
	</div>;
  ...
  const landingPageContent = <div
    ref={sidebarRef}
    className="ainative-w-full ainative-h-full ainative-max-h-full ainative-flex ainative-flex-col ainative-overflow-auto ainative-px-4">
    ...
    <ErrorBoundary>
				<div className="ainative-pt-8 ainative-mb-2 ainative-text-ainative-fg-3 ainative-text-root ainative-select-none ainative-pointer-events-none">Previous Threads</div>
				<PastThreadsList />
			</ErrorBoundary> :

    <ErrorBoundary>
				<div className="ainative-pt-8 ainative-mb-2 ainative-text-ainative-fg-3 ainative-text-root ainative-select-none ainative-pointer-events-none">Suggestions</div>
  ...
  const threadPageContent = <div
    ref={sidebarRef}
    className="ainative-w-full ainative-h-full ainative-flex ainative-flex-col ainative-overflow-hidden">
