...
const ButtonLeftTextRightOption = ({ text, leftButton }: {text: string;leftButton?: React.ReactNode;}) => {
  ...
  return <div className="ainative-flex ainative-items-center ainative-text-ainative-fg-3 ainative-px-3 ainative-py-0.5 ainative-rounded-sm ainative-overflow-hidden ainative-gap-2">
  ...
  return (
    ...
    onMouseUp={() => {
      ...
      mouseDownInsideModal.current = false;
			...
			{/* MODAL */}
			<div
        className="ainative-bg-ainative-bg-1 ainative-rounded-md ainative-p-4 ainative-max-w-xl ainative-w-full ainative-shadow-xl ainative-overflow-y-auto ainative-max-h-[90vh]"
        ...
        onMouseDown={(e) => {
          ...
          e.stopPropagation();
				...
				<div className="ainative-flex ainative-justify-between ainative-items-center ainative-mb-4">
					...
					<button
            onClick={onClose}
            className="ainative-text-ainative-fg-3 hover:ainative-text-ainative-fg-1">
				...
				<div className="ainative-text-sm ainative-text-ainative-fg-3 ainative-mb-4">
          ...
          `AINative Studio recognizes ${modelName} ("${recognizedModelName}").`}
				...
				<div className="ainative-flex ainative-items-center ainative-gap-2 ainative-mb-4">
					...
					<span className="ainative-text-ainative-fg-3 ainative-text-sm">Override model defaults</span>
				</div>

				{/* Informational link */}
				{overrideEnabled && <div className="ainative-text-sm ainative-text-ainative-fg-3 ainative-mb-4">
          ...
          key={overrideEnabled + ''}
          ref={textAreaRef}
          className={`ainative-w-full ainative-min-h-[200px] ainative-p-2 ainative-rounded-sm ainative-border ainative-border-ainative-border-2 ainative-bg-ainative-bg-2 ainative-resize-none ainative-font-mono ainative-text-sm ${!overrideEnabled ? "ainative-text-ainative-fg-3" : ""}`}
        ...
        <div className="ainative-text-red-500 ainative-mt-2 ainative-text-sm">{errorMsg}</div>
				...
				<div className="ainative-flex ainative-justify-end ainative-gap-2 ainative-mt-4">
					<AINativeButtonBgDarken onClick={onClose} className="ainative-px-3 ainative-py-1">
						Cancel
					</AINativeButtonBgDarken>
					<AINativeButtonBgDarken
            onClick={onSave}
...
export const ModelDump = ({ filteredProviders }: {filteredProviders?: ProviderName[];}) => {
  ...
  return <div className="">
      ...
      className={`ainative-flex ainative-items-center ainative-justify-between ainative-gap-4 hover:ainative-bg-black/10 dark:hover:ainative-bg-gray-300/10 ainative-py-1 ainative-px-3 ainative-rounded-sm ainative-overflow-hidden ainative-cursor-default ainative-truncate ainative-group `}>
          ...
          <div className="ainative-w-5 ainative-flex ainative-items-center ainative-justify-center">
              ...
              className={`${hasOverrides ? "" : "ainative-opacity-0 group-hover:ainative-opacity-100"} ainative-transition-opacity`}>
								...
								<Plus size={12} className="ainative-text-ainative-fg-3 ainative-opacity-50" />
							</button>
						</div>
          }
            ...
            data-tooltip-content={tooltipName} />
              ...
              className={`${hasOverrides ? "" : "ainative-opacity-0 group-hover:ainative-opacity-100"} ainative-transition-opacity`}>
							...
							<X size={12} className="ainative-text-ainative-fg-3 ainative-opacity-50" />
						</button>}
					</div>
				</div>
			</div>;
    ...
    <div className="ainative-mt-4">
          ...
          className="ainative-text-ainative-fg-4">
    ...
    <div
      className="ainative-text-ainative-fg-4 ainative-flex ainative-flex-nowrap ainative-text-nowrap ainative-items-center hover:ainative-brightness-110 ainative-cursor-pointer ainative-mt-4"
...
const FastApplyMethodDropdown = () => {
  ...
  return <AINativeCustomDropdownBox
    className="ainative-text-xs ainative-text-ainative-fg-3 ainative-bg-ainative-bg-1 ainative-border ainative-border-ainative-border-1 ainative-rounded ainative-p-0.5 ainative-px-1"
    options={options}
...
export const OllamaSetupInstructions = ({ sayWeAutoDetect }: {sayWeAutoDetect?: boolean;}) => {
  return <div className="prose-p:ainative-my-0 prose-ol:ainative-list-decimal prose-p:ainative-py-0 prose-ol:ainative-my-0 prose-ol:ainative-py-0 prose-span:ainative-my-0 prose-span:ainative-py-0 ainative-text-ainative-fg-3 ainative-text-sm ainative-list-decimal ainative-select-text">
...
const RedoOnboardingButton = ({ className }: {className?: string;}) => {
  ...
  const voidSettingsService = accessor.get('IAINativeSettingsService');
  return <div
    className={`ainative-text-ainative-fg-4 ainative-flex ainative-flex-nowrap ainative-text-nowrap ainative-items-center hover:ainative-brightness-110 ainative-cursor-pointer ${className}`}
...
export const ToolApprovalTypeSwitch = ({ approvalType, size, desc }: {approvalType: ToolApprovalType;size: "xxs" | "xs" | "sm" | "sm+" | "md";desc: string;}) => {
  ...
  return <>
      ...
      onChange={(newVal) => onToggleAutoApprove(approvalType, newVal)} />
		...
		<span className="ainative-text-ainative-fg-3 ainative-text-xs">{desc}</span>
	</>;
};
...
export const OneClickSwitchButton = ({ fromEditor = 'VS Code', className = '' }: {fromEditor?: TransferEditorType;className?: string;}) => {
  ...
  return <>
		<AINativeButtonBgDarken className={`ainative-max-w-48 ainative-p-4 ${className}`} disabled={transferState.type !== 'done'} onClick={onClick}>
...
const MCPServerComponent = ({ name, server }: {name: string;server: MCPServer;}) => {
  ...
  return (
    <div className="ainative-border ainative-border-ainative-border-2 ainative-bg-ainative-bg-1 ainative-py-3 ainative-px-4 ainative-rounded-sm ainative-my-2">
          ...
          server.status === 'offline' ? "ainative-bg-ainative-fg-3" : ""} `}>

          </div>

					{/* Server name */}
					<div className="ainative-text-sm ainative-font-medium ainative-text-ainative-fg-1">{name}</div>
				</div>
      ...
      <div className="ainative-mt-3">
          ...
          <span
            key={tool.name}
            className="ainative-px-2 ainative-py-0.5 ainative-bg-ainative-bg-2 ainative-text-ainative-fg-3 ainative-rounded-sm ainative-text-xs"
          ...
          <span className="ainative-text-xs ainative-text-ainative-fg-3">No tools available</span>
          }
					</div>
				</div>
      }
			...
			{isOn && server.command &&
      <div className="ainative-mt-3">
					<div className="ainative-text-xs ainative-text-ainative-fg-3 ainative-mb-1">Command:</div>
					<div className="ainative-px-2 ainative-py-1 ainative-bg-ainative-bg-2 ainative-text-xs ainative-font-mono ainative-overflow-x-auto ainative-whitespace-nowrap ainative-text-ainative-fg-2 ainative-rounded-sm">
						{server.command}
					</div>
				</div>
      }
...
const MCPServersList = () => {
  ...
  if (mcpServiceState.error) {
    content = <div className="ainative-text-ainative-fg-3 ainative-text-sm ainative-mt-2">
...
export const Settings = () => {
  ...
  return (
    <div className={`ainative-scope ${isDark ? "ainative-dark" : ""}`} style={{ height: '100%', width: '100%', overflow: 'auto' }}>
            ...
            <button
              ...
              selectedSection === tab ? "ainative-bg-[#0e70c0]/80 ainative-text-white ainative-font-medium ainative-shadow-sm" : "ainative-bg-ainative-bg-2 hover:ainative-bg-ainative-bg-2/80 ainative-text-ainative-fg-1"} `}>
				...
				<main className="ainative-flex-1 ainative-p-6 ainative-select-none">
					...
					<div className="ainative-max-w-3xl">
						...
						{/* Account Section */}
						<ErrorBoundary>
							<div className="ainative-border ainative-border-ainative-border-2 ainative-bg-ainative-bg-1 ainative-py-4 ainative-px-4 ainative-rounded-md ainative-my-4">
                ...
                <div className="ainative-flex ainative-items-center ainative-gap-4">
										<div className="ainative-flex ainative-items-center ainative-gap-3 ainative-flex-1">
											<div className="ainative-w-12 ainative-h-12 ainative-rounded-full ainative-bg-ainative-bg-2 ainative-flex ainative-items-center ainative-justify-center ainative-overflow-hidden">
												{auth.user?.name ?
                      <img
                      ...
                      <span className="ainative-text-ainative-fg-3 ainative-text-xl">👤</span>
                      }
											</div>
											<div className="ainative-flex ainative-flex-col">
												<div className="ainative-text-ainative-fg-1 ainative-font-medium">{auth.user?.name || auth.user?.email}</div>
												<div className="ainative-text-ainative-fg-3 ainative-text-sm">{auth.user?.email}</div>
											</div>
										</div>
										<AINativeButtonBgDarken
                ...
                <div className="ainative-flex ainative-flex-col ainative-gap-2">
										<p className="ainative-text-ainative-fg-3 ainative-text-sm ainative-mb-2">
						...
						<div className="ainative-flex ainative-flex-col ainative-gap-12">
							...
							<div className={shouldShowTab('localProviders') ? `` : "ainative-hidden"}>
								<ErrorBoundary>
									...
									<h3 className={`ainative-text-ainative-fg-3 ainative-mb-2`}>{`AINative Studio can access any model that you host locally. We automatically detect your local models by default.`}</h3>
							...
							<div className={shouldShowTab('providers') ? `` : "ainative-hidden"}>
								<ErrorBoundary>
									...
									<h3 className={`ainative-text-ainative-fg-3 ainative-mb-2`}>{`AINative Studio can access models from Anthropic, OpenAI, OpenRouter, and more.`}</h3>
									...
									<div className="ainative-flex ainative-flex-col ainative-gap-y-8 ainative-my-4">
										<ErrorBoundary>
											...
											<div>
												...
												<div className="ainative-text-sm ainative-text-ainative-fg-3 ainative-mt-1">
													<span>
														Experimental.{' '}
													</span>
													<span
                            ...
                            data-tooltip-class-name='ainative-max-w-[20px]'>
                                ...
                                onChange={(newVal) => voidSettingsService.setGlobalSetting('enableAutocomplete', newVal)} />
															...
															<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{settingsState.globalSettings.enableAutocomplete ? 'Enabled' : 'Disabled'}</span>
														</div>
													</ErrorBoundary>
													...
													<ErrorBoundary>
														<div className={`ainative-my-2 ${!settingsState.globalSettings.enableAutocomplete ? "ainative-hidden" : ""}`}>
															<ModelDropdown featureName={'Autocomplete'} className="ainative-text-xs ainative-text-ainative-fg-3 ainative-bg-ainative-bg-1 ainative-border ainative-border-ainative-border-1 ainative-rounded ainative-p-0.5 ainative-px-1" />
														</div>
													</ErrorBoundary>
										...
										<ErrorBoundary>
											...
											<div className="ainative-w-full">
												...
												<div className="ainative-text-sm ainative-text-ainative-fg-3 ainative-mt-1">Settings that control the behavior of the Apply button.</div>
                              ...
                              onChange={(newVal) => voidSettingsService.setGlobalSetting('syncApplyToChat', newVal)} />
														...
														<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{settingsState.globalSettings.syncApplyToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`ainative-my-2 ${settingsState.globalSettings.syncApplyToChat ? "ainative-hidden" : ""}`}>
														<ModelDropdown featureName={'Apply'} className="ainative-text-xs ainative-text-ainative-fg-3 ainative-bg-ainative-bg-1 ainative-border ainative-border-ainative-border-1 ainative-rounded ainative-p-0.5 ainative-px-1" />
													</div>
												</div>
										...
										<div>
											...
											<div className="ainative-text-sm ainative-text-ainative-fg-3 ainative-mt-1">{`Tools are functions that LLMs can call. Some tools require user approval.`}</div>
                            ...
                            return <div key={approvalType} className="ainative-flex ainative-items-center ainative-gap-x-2 ainative-my-2">
                              ...
                              onChange={(newVal) => voidSettingsService.setGlobalSetting('includeToolLintErrors', newVal)} />
														...
														<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{settingsState.globalSettings.includeToolLintErrors ? 'Fix lint errors' : `Fix lint errors`}</span>
													</div>
												</ErrorBoundary>
                              ...
                              onChange={(newVal) => voidSettingsService.setGlobalSetting('autoAcceptLLMChanges', newVal)} />
														...
														<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">Auto-accept LLM changes</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<div className="ainative-w-full">
											...
											<div className="ainative-text-sm ainative-text-ainative-fg-3 ainative-mt-1">{`Settings that control the visibility of AINative Studio suggestions in the code editor.`}</div>
                              ...
                              onChange={(newVal) => voidSettingsService.setGlobalSetting('showInlineSuggestions', newVal)} />
														...
														<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{settingsState.globalSettings.showInlineSuggestions ? 'Show suggestions on select' : 'Show suggestions on select'}</span>
													</div>
												</ErrorBoundary>
											</div>
										</div>
										...
										<ErrorBoundary>
											...
											<div className="ainative-w-full">
												...
												<div className="ainative-text-sm ainative-text-ainative-fg-3 ainative-mt-1">Settings that control the behavior of the commit message generator.</div>
                              ...
                              onChange={(newVal) => voidSettingsService.setGlobalSetting('syncSCMToChat', newVal)} />
														...
														<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{settingsState.globalSettings.syncSCMToChat ? 'Same as Chat model' : 'Different model'}</span>
													</div>

													{/* Model Dropdown */}
													<div className={`ainative-my-2 ${settingsState.globalSettings.syncSCMToChat ? "ainative-hidden" : ""}`}>
														<ModelDropdown featureName={'SCM'} className="ainative-text-xs ainative-text-ainative-fg-3 ainative-bg-ainative-bg-1 ainative-border ainative-border-ainative-border-1 ainative-rounded ainative-p-0.5 ainative-px-1" />
													</div>
												</div>
							...
							<div className={`${shouldShowTab('general') ? `` : "ainative-hidden"} ainative-flex ainative-flex-col ainative-gap-12`}>
								...
								<div>
									<ErrorBoundary>
										...
										<h4 className="ainative-text-ainative-fg-3 ainative-mb-4">{`Transfer your editor settings into AINative Studio.`}</h4>
								...
								<div>
									...
									<h4 className="ainative-text-ainative-fg-3 ainative-mb-4">{`Transfer AINative Studio's settings and chats in and out of AINative Studio.`}</h4>
									<div className="ainative-flex ainative-flex-col ainative-gap-8">
										...
										<div className="ainative-flex ainative-flex-col ainative-gap-2 ainative-max-w-48 ainative-w-full">
											...
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1 ainative-w-full" onClick={() => {fileInputSettingsRef.current?.click();}}>
												Import Settings
											</AINativeButtonBgDarken>
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1 ainative-w-full" onClick={() => onDownload('Settings')}>
												Export Settings
											</AINativeButtonBgDarken>
										...
										<div className="ainative-flex ainative-flex-col ainative-gap-2 ainative-max-w-48 ainative-w-full">
											...
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1 ainative-w-full" onClick={() => {fileInputChatsRef.current?.click();}}>
												Import Chats
											</AINativeButtonBgDarken>
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1 ainative-w-full" onClick={() => onDownload('Chats')}>
												Export Chats
											</AINativeButtonBgDarken>
								...
								<div>
									...
									<h4 className={`ainative-text-ainative-fg-3 ainative-mb-4`}>{`IDE settings, keyboard settings, and theme customization.`}</h4>

									<ErrorBoundary>
										<div className="ainative-flex ainative-flex-col ainative-gap-2 ainative-justify-center ainative-max-w-48 ainative-w-full">
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1" onClick={() => {commandService.executeCommand('workbench.action.openSettings');}}>
												General Settings
											</AINativeButtonBgDarken>
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1" onClick={() => {commandService.executeCommand('workbench.action.openGlobalKeybindings');}}>
												Keyboard Settings
											</AINativeButtonBgDarken>
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1" onClick={() => {commandService.executeCommand('workbench.action.selectTheme');}}>
												Theme Settings
											</AINativeButtonBgDarken>
											<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1" onClick={() => {nativeHostService.showItemInFolder(environmentService.logsHome.fsPath);}}>
												Open Logs
											</AINativeButtonBgDarken>
										</div>
									</ErrorBoundary>
								...
								<div className="ainative-max-w-[600px]">
									...
									<h4 className={`ainative-text-ainative-fg-3 ainative-mb-4`}>Very basic anonymous usage tracking helps us keep AINative Studio running smoothly. You may opt out below. Regardless of this setting, AINative Studio never sees your code, messages, or API keys.</h4>
                          ...
                          onChange={(newVal) => {
                            ...
                            metricsService.capture(`Set metrics opt-out to ${newVal}`, {}); // this only fires if it's enabled, so it's fine to have here
												...
												<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">{'Opt-out (requires restart)'}</span>
											</div>
										</ErrorBoundary>
									</div>
								</div>
								...
								<div className="ainative-max-w-[600px]">
									<h2 className={`ainative-text-3xl ainative-mb-2`}>Global Rules</h2>
									<h4 className={`ainative-text-ainative-fg-3 ainative-mb-4`}>
...
Alternatively, place a \`.ainativerules\` file in the root of your workspace.
                          ...
                          onChange={(newValue) => {
                            voidSettingsService.setGlobalSetting('disableSystemMessage', newValue);
												...
												<span className="ainative-text-ainative-fg-3 ainative-text-xs ainative-pointer-events-none">
													{'Disable system message'}
												</span>
											</div>
										</ErrorBoundary>
										<div className="ainative-text-ainative-fg-3 ainative-text-xs ainative-mt-1">
...
Use Model Context Protocol to provide Agent mode with more tools.
							`} chatMessageLocation={undefined} />
									...
									<div className="ainative-my-2">
										<AINativeButtonBgDarken className="ainative-px-4 ainative-py-1 ainative-w-full ainative-max-w-48" onClick={async () => {await mcpService.revealMCPConfigFile();}}>
											Add MCP Server
										</AINativeButtonBgDarken>
									</div>
