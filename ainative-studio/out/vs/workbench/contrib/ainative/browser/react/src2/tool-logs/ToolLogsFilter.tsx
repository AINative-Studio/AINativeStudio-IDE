...
export const ToolLogsFilter: React.FC<ToolLogsFilterProps> = ({ filter, onChange }) => {
  ...
  return (
    <div className="ainative-tool-logs-filter">
			<div className="ainative-tool-logs-filter-header">
				<button
          className="ainative-tool-logs-filter-toggle"
          onClick={() => setExpanded(!expanded)}>

					<span className={`ainative-codicon ainative-codicon-chevron-${expanded ? "ainative-down" : "ainative-right"}`}></span>
					<span>Filters</span>
					{(localFilter.toolTypes?.length || localFilter.statuses?.length || localFilter.searchQuery) &&
          <span className="ainative-tool-logs-filter-badge">
				...
				<input
          type="search"
          className="ainative-tool-logs-search"
          ...
          onChange={(e) => handleSearchChange(e.target.value)} />
			...
			{expanded &&
      <div className="ainative-tool-logs-filter-content">
					<div className="ainative-tool-logs-filter-section">
						<label>Tool Types</label>
						<div className="ainative-tool-logs-filter-chips">
							{toolTypes.map((toolType) =>
            <button
              key={toolType}
              className={`ainative-tool-logs-chip ${localFilter.toolTypes?.includes(toolType) ? "ainative-active" : ""}`}
            ...
            <button
              key={status}
              className={`ainative-tool-logs-chip ainative-status-${status} ${localFilter.statuses?.includes(status) ? "ainative-active" : ""}`}
              onClick={() => handleStatusToggle(status)}>
					...
					<div className="ainative-tool-logs-filter-section">
						...
						<div className="ainative-tool-logs-filter-dates">
							<input
              type="datetime-local"
              ...
              placeholder="End date" />
					...
					<div className="ainative-tool-logs-filter-section">
						...
						<div className="ainative-tool-logs-filter-range">
							<input
              type="number"
              placeholder="Min"
