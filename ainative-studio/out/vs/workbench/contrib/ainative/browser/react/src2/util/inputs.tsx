...
export const WidgetComponent = <CtorParams extends any[], Instance>({ ctor, propsFn, dispose, onCreateInstance, children, className
  ...
  return <div ref={containerRef} className={className === undefined ? `ainative-w-full` : className}>{children}</div>;
};
...
export const AINativeInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
  ...
  return <>
      ...
      className={`ainative-w-full ainative-resize-none ainative-max-h-[500px] ainative-overflow-y-auto ainative-text-ainative-fg-1 placeholder:ainative-text-ainative-fg-3 ${className}`}
      style={{
    ...
    <div
      ...
      className="ainative-z-[100] ainative-border-ainative-border-3 ainative-bg-ainative-bg-2-alt ainative-border ainative-rounded ainative-shadow-lg ainative-flex ainative-flex-col ainative-overflow-hidden"
      style={{
        position: strategy,
        top: y ?? 0,
        left: x ?? 0,
      ...
      onWheel={(e) => e.stopPropagation()}>
				...
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="ainative-px-2 ainative-py-1 ainative-text-ainative-fg-1 ainative-bg-ainative-bg-2-alt ainative-border-b ainative-border-ainative-border-3 ainative-sticky ainative-top-0 ainative-bg-ainative-bg-1 ainative-z-10 ainative-select-none ainative-pointer-events-none">
					{optionText ?
        ...
        <div className="ainative-opacity-50">Enter text to filter...</div>
        }
				</div>}


				{/* Options list */}
				<div className="ainative-max-h-[400px] ainative-w-full ainative-max-w-full ainative-overflow-y-auto ainative-overflow-x-auto">
					<div className="ainative-w-max ainative-min-w-full ainative-flex ainative-flex-col ainative-gap-0 ainative-text-nowrap ainative-flex-nowrap">
						{options.length === 0 ?
          <div className="ainative-text-ainative-fg-3 ainative-px-3 ainative-py-0.5">No results found</div> :
          options.map((o, oIdx) => {
            ...
            return (
              ...
              <div
                ...
                key={o.fullName}
                className={` ainative-flex ainative-items-center ainative-gap-2 ainative-px-3 ainative-py-1 ainative-cursor-pointer ${


                oIdx === optionIdx ? "ainative-bg-blue-500 ainative-text-white/80" : "ainative-bg-ainative-bg-2-alt ainative-text-ainative-fg-1"} `}
                ...
                onMouseMove={() => {setOptionIdx(oIdx);}}>
										...
										{o.fullName && o.fullName !== o.abbreviatedName && <span className="ainative-opacity-60 ainative-text-sm">{o.fullName}</span>}
...
export const AINativeSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps
  ...
  return (
    <input
      ...
      placeholder={placeholder}
      disabled={disabled}
      className={`ainative-w-full ainative-resize-none ainative-bg-ainative-bg-1 ainative-text-ainative-fg-1 placeholder:ainative-text-ainative-fg-3 ainative-border ainative-border-ainative-border-2 focus:ainative-border-ainative-border-1 ${
      compact ? "ainative-py-1 ainative-px-2" : "ainative-py-2 ainative-px-4 "} ainative-rounded ${

      disabled ? "ainative-opacity-50 ainative-cursor-not-allowed" : ""} ${
      className}`}
      style={{
...
export const AINativeInputBox = ({ onChangeText, onCreateInstance, inputBoxRef, placeholder, isPasswordField, multiline
  ...
  return <WidgetComponent
    className=" ainative-bg-ainative-bg-1 ainative-force-child-placeholder-ainative-fg-1 "
...
export const AINativeSlider = ({
  ...
  return (
    <div className={`ainative-inline-flex ainative-items-center ainative-flex-shrink-0 ${className}`}>
			{/* Outer container with padding to account for thumb overhang */}
			<div className={`ainative-relative ainative-flex-shrink-0 ${disabled ? "ainative-opacity-25" : ""}`}
      style={{
        width
        ...
        // paddingRight: thumbSizePx / 2
				...
				<div className="ainative-relative ainative-w-full">
					...
					<div
            className="ainative-absolute ainative-w-full ainative-cursor-pointer"
            style={{
              height: '16px',
              top: '50%',
            ...
            onClick={handleTrackClick} />
					...
					{/* Track */}
					<div
            className={`ainative-relative ${size === 'xxs' ? "ainative-h-0.5" :
            size === 'xs' ? "ainative-h-1" :
            size === 'sm' ? "ainative-h-1.5" :
            size === 'sm+' ? "ainative-h-2" : "ainative-h-2.5"} ainative-bg-ainative-bg-2 ainative-rounded-full ainative-cursor-pointer`}

            onClick={handleTrackClick}>

						{/* Filled part of track */}
						<div
              className={`ainative-absolute ainative-left-0 ${size === 'xxs' ? "ainative-h-0.5" :
              size === 'xs' ? "ainative-h-1" :
              size === 'sm' ? "ainative-h-1.5" :
              size === 'sm+' ? "ainative-h-2" : "ainative-h-2.5"} ainative-bg-ainative-fg-1 ainative-rounded-full`}
              ...
              style={{ width: `${percentage}%` }} />
					...
					{/* Thumb */}
					<div
            className={`ainative-absolute ainative-top-1/2 ainative-transform -ainative-translate-x-1/2 -ainative-translate-y-1/2 ${
            size === 'xxs' ? "ainative-h-2 ainative-w-2" :
            size === 'xs' ? "ainative-h-2.5 ainative-w-2.5" :
            size === 'sm' ? "ainative-h-3 ainative-w-3" :
            size === 'sm+' ? "ainative-h-3.5 ainative-w-3.5" : "ainative-h-4 ainative-w-4"} ainative-bg-ainative-fg-1 ainative-rounded-full ainative-shadow-md ${

            disabled ? "ainative-cursor-not-allowed" : "ainative-cursor-grab active:ainative-cursor-grabbing"} ainative-border ainative-border-ainative-fg-1`}
...
export const AINativeSwitch = ({
  ...
  return (
    <label className="ainative-inline-flex ainative-items-center" {...props}>
			<div
        onClick={() => !disabled && onChange(!value)}
        className={` ainative-cursor-pointer ainative-relative ainative-inline-flex ainative-items-center ainative-rounded-full ainative-transition-colors ainative-duration-200 ainative-ease-in-out ${


        value ? "ainative-bg-zinc-900 dark:ainative-bg-white" : "ainative-bg-white dark:ainative-bg-zinc-600"} ${
        disabled ? "ainative-opacity-25" : ""} ${
        size === 'xxs' ? "ainative-h-3 ainative-w-5" : ""} ${
        size === 'xs' ? "ainative-h-4 ainative-w-7" : ""} ${
        size === 'sm' ? "ainative-h-5 ainative-w-9" : ""} ${
        size === 'sm+' ? "ainative-h-5 ainative-w-10" : ""} ${
        size === 'md' ? "ainative-h-6 ainative-w-11" : ""} `}>


				<span
          className={` ainative-inline-block ainative-transform ainative-rounded-full ainative-bg-white dark:ainative-bg-zinc-900 ainative-shadow ainative-transition-transform ainative-duration-200 ainative-ease-in-out ${

          size === 'xxs' ? "ainative-h-2 ainative-w-2" : ""} ${
          size === 'xs' ? "ainative-h-2.5 ainative-w-2.5" : ""} ${
          size === 'sm' ? "ainative-h-3 ainative-w-3" : ""} ${
          size === 'sm+' ? "ainative-h-3.5 ainative-w-3.5" : ""} ${
          size === 'md' ? "ainative-h-4 ainative-w-4" : ""} ${
          size === 'xxs' ? value ? "ainative-translate-x-2.5" : "ainative-translate-x-0.5" : ""} ${
          size === 'xs' ? value ? "ainative-translate-x-3.5" : "ainative-translate-x-0.5" : ""} ${
          size === 'sm' ? value ? "ainative-translate-x-5" : "ainative-translate-x-1" : ""} ${
          size === 'sm+' ? value ? "ainative-translate-x-6" : "ainative-translate-x-1" : ""} ${
          size === 'md' ? value ? "ainative-translate-x-6" : "ainative-translate-x-1" : ""} `} />
...
export const AINativeCheckBox = ({ label, value, onClick, className }: {label: string;value: boolean;onClick: (checked: boolean) => void;className?: string;}) => {
...
export const AINativeCustomDropdownBox = <T extends NonNullable<any>,>({
  options,
  selectedOption,
  onChangeOption,
  ...
  return (
    <div className={`ainative-inline-block ainative-relative ${className}`}>
			{/* Hidden measurement div */}
			<div
        ref={measureRef}
        className="ainative-opacity-0 ainative-pointer-events-none ainative-absolute -ainative-left-[999999px] -ainative-top-[999999px] ainative-flex ainative-flex-col"
        aria-hidden="true">
          ...
          return (
            <div key={optionName + optionDetail} className="ainative-flex ainative-items-center ainative-whitespace-nowrap">
							<div className="ainative-w-4" />
							<span className="ainative-flex ainative-justify-between ainative-w-full">
        ...
        className="ainative-flex ainative-items-center ainative-h-4 ainative-bg-transparent ainative-whitespace-nowrap hover:ainative-brightness-90 ainative-w-full"
        onClick={() => setIsOpen(!isOpen)}>

				<span className={`ainative-truncate ${arrowTouchesText ? "ainative-mr-1" : ""}`}>
					{getOptionDisplayName(selectedOption)}
				</span>
				<svg
          className={`ainative-size-3 ainative-flex-shrink-0 ${arrowTouchesText ? "" : "ainative-ml-auto"}`}
          viewBox="0 0 12 12"
          fill="none">
      ...
      <div
        ref={refs.setFloating}
        className="ainative-z-[100] ainative-bg-ainative-bg-1 ainative-border-ainative-border-3 ainative-border ainative-rounded ainative-shadow-lg"
        style={{
          position: strategy,
          top: y ?? 0,
          left: x ?? 0,
        ...
        onWheel={(e) => e.stopPropagation()}>
        <div className="ainative-overflow-auto ainative-max-h-80">
            ...
            return (
              <div
                key={optionName}
                className={`ainative-flex ainative-items-center ainative-px-2 ainative-py-1 ainative-pr-4 ainative-cursor-pointer ainative-whitespace-nowrap ainative-transition-all ainative-duration-100 ${

                thisOptionIsSelected ? "ainative-bg-blue-500 ainative-text-white/80" : "hover:ainative-bg-blue-500 hover:ainative-text-white/80"} `}
                ...
                onClick={() => {
                  ...
                  setIsOpen(false);
									...
									<div className="ainative-w-4 ainative-flex ainative-justify-center ainative-flex-shrink-0">
										{thisOptionIsSelected &&
                  <svg className="ainative-size-3" viewBox="0 0 12 12" fill="none">
												<path
                      d="M10 3L4.5 8.5L2 6"
                      stroke="currentColor"
                      strokeWidth="1.5"
                      ...
                      strokeLinejoin="round" />
									...
									<span className="ainative-flex ainative-justify-between ainative-items-center ainative-w-full ainative-gap-x-1">
										<span>{optionName}</span>
										<span className="ainative-opacity-60">{optionDetail}</span>
									</span>
								</div>);
...
export const _AINativeSelectBox = <T,>({ onChangeSelection, onCreateInstance, selectBoxRef, options, className
  ...
  return <WidgetComponent
    className={` select-child-restyle [&_select]:!void-text-ainative-fg-3 [&_select]:!void-text-xs !ainative-text-ainative-fg-3 ${
...
export const BlockCode = ({ initValue, language, maxHeight, showScrollbars }: BlockCodeProps) => {
  ...
  return <div ref={divRef} className="ainative-relative ainative-z-0 ainative-px-2 ainative-py-1 ainative-bg-ainative-bg-3">
		<WidgetComponent
...
export const AINativeButtonBgDarken = ({ children, disabled, onClick, className }: {children: React.ReactNode;disabled?: boolean;onClick: () => void;className?: string;}) => {
  return <button disabled={disabled}
  className={`ainative-px-3 ainative-py-1 ainative-bg-black/10 dark:ainative-bg-white/10 ainative-rounded-sm ainative-overflow-hidden ainative-whitespace-nowrap ainative-flex ainative-items-center ainative-justify-center ${className || ''}`}
  onClick={onClick}>
    {children}</button>;
};

// export const AINativeScrollableElt = ({ options, children }: { options: ScrollableElementCreationOptions, children: React.ReactNode }) => {
...
// export const AINativeSelectBox = <T,>({ onChangeSelection, initVal, selectBoxRef, options }: {
// 	initVal: T;
...
// export const AINativeCheckBox = ({ onChangeChecked, initVal, label, checkboxRef, }: {
...
const SingleDiffEditor = ({ block, lang }: {block: ExtractedSearchReplaceBlock;lang: string | undefined;}) => {
  ...
  return (
    <div className="ainative-w-full ainative-bg-ainative-bg-3 bg-editor-style-override" ref={divRef} />);
 ...
 */
export const AINativeDiffEditor = ({ uri, searchReplaceBlocks, language }: {uri?: any;searchReplaceBlocks: string;language?: string;}) => {
  const accessor = useAccessor();
  const languageService = accessor.get('ILanguageService');
  ...
  if (blocks.length === 0) {
    return <div className="ainative-w-full ainative-p-4 ainative-text-ainative-fg-4 ainative-text-sm">No changes found</div>;
  }

  // Display all blocks
  return (
    <div className="ainative-w-full ainative-flex ainative-flex-col ainative-gap-2">
			{blocks.map((block, index) =>
      <div key={index} className="ainative-w-full">
					{blocks.length > 1 &&
        <div className="ainative-text-ainative-fg-4 ainative-text-xs ainative-mb-1 ainative-px-1">
