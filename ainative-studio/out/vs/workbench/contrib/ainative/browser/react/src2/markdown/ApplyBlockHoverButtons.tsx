...
import { isFeatureNameDisabled } from '../../../../common/ainativeSettingsTypes.js';
...
export const IconShell1 = ({ onClick, Icon, disabled, className, ...props }: IconButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  return <button
    ...
    // border border-ainative-border-1 rounded
    className={` ainative-size-[18px] ainative-p-[2px] ainative-flex ainative-items-center ainative-justify-center ainative-text-sm ainative-text-ainative-fg-3 hover:ainative-brightness-110 disabled:ainative-opacity-50 disabled:ainative-cursor-not-allowed ${
...
export const StatusIndicator = ({ indicatorColor, title, className, ...props }: {indicatorColor: IndicatorColor;title?: React.ReactNode;className?: string;} & React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <div className={`ainative-flex ainative-flex-row ainative-text-ainative-fg-3 ainative-text-xs ainative-items-center ainative-gap-1.5 ${className}`} {...props}>
			{title && <span className="ainative-opacity-80">{title}</span>}
			<div
        className={` ainative-size-1.5 ainative-rounded-full ainative-border ${
        indicatorColor === 'dark' ? "ainative-bg-[rgba(0,0,0,0)] ainative-border-ainative-border-1" :
        indicatorColor === 'orange' ? "ainative-bg-orange-500 ainative-border-orange-500 ainative-shadow-[0_0_4px_0px_rgba(234,88,12,0.6)]" :
        indicatorColor === 'green' ? "ainative-bg-green-500 ainative-border-green-500 ainative-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" :
        indicatorColor === 'yellow' ? "ainative-bg-yellow-500 ainative-border-yellow-500 ainative-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" : "ainative-bg-ainative-border-1 ainative-border-ainative-border-1"} `} />
...
const ApplyButtonsForEdit = ({
  ...
  const onClickSubmit = useCallback(async () => {
    ...
    if (!applyDonePromise) {
      notificationService.info(`AINative Error: We couldn't run Apply here. ${uri === 'current' ? 'This Apply block wants to run on the current file, but you might not have a file open.' : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`);
    }
    ...
    applyDonePromise?.catch((e) => {
      ...
      notificationService.info(`AINative Error: There was a problem running Apply: ${e}.`);
...
export const BlockCodeApplyWrapper = ({
  ...
  <ListableToolItem
    name={<span className="ainative-not-italic">{getBasename(uri.fsPath)}</span>}
    isSmall={true}
    showDot={false}
  ...
  return <div className="ainative-border ainative-border-ainative-border-3 ainative-rounded ainative-overflow-hidden ainative-bg-ainative-bg-3 ainative-my-1">
		{/* header */}
		<div className=" ainative-select-none ainative-flex ainative-justify-between ainative-items-center ainative-py-1 ainative-px-2 ainative-border-b ainative-border-ainative-border-3 ainative-cursor-default">
			<div className="ainative-flex ainative-items-center">
				<StatusIndicatorForApplyButton uri={uri} applyBoxId={applyBoxId} />
				<span className="ainative-text-[13px] ainative-font-light ainative-text-ainative-fg-3">
					{name}
				</span>
			</div>
			<div className={`${canApply ? "" : "ainative-hidden"} ainative-flex ainative-items-center ainative-gap-1`}>
