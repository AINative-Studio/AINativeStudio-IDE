...
const LatexRender = ({ latex }: {latex: string;}) => {
  return <span className="ainative-katex-error ainative-text-red-500">{latex}</span>;
  // try {
  // 	let formula = latex;
...
const Codespan = ({ text, className, onClick, tooltip }: {text: string;className?: string;onClick?: () => void;tooltip?: string;}) => {
  ...
  return <code
    className={`ainative-font-mono ainative-font-medium ainative-rounded-sm ainative-bg-ainative-bg-1 ainative-px-1 ${className}`}
    onClick={onClick}
    {...tooltip ? {
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: {text: string;rawText: string;chatMessageLocation: ChatMessageLocation;}) => {
  ...
  return <Codespan
    text={displayText}
    onClick={onClick}
    className={link ? "ainative-underline hover:ainative-brightness-90 ainative-transition-all ainative-duration-200 ainative-cursor-pointer" : ""}
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: {token: Token | string;inPTag?: boolean;codeURI?: URI;chatMessageLocation?: ChatMessageLocation;tokenIdx: string;} & RenderTokenOptions): React.ReactNode => {
  ...
  if (t.type === 'table') {
    ...
    // return (
    // 	<div>
    // 		<table className={'min-w-full border border-ainative-bg-2'}>
    // 			<thead>
    // 				<tr className='bg-ainative-bg-1'>
    // 					{t.header.map((cell: any, index: number) => (
    // 						<th
    // 							key={index}
    // 							className='px-4 py-2 border border-ainative-bg-2 font-semibold'
    ...
    // 					<tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-ainative-bg-1'}>
    // 						{row.map((cell: any, cellIndex: number) => (
    // 							<td
    // 								key={cellIndex}
    // 								className={'px-4 py-2 border border-ainative-bg-2'}
  ...
  if (t.type === 'paragraph') {
    ...
    if (latexSegments.length !== 0) {
      if (inPTag) {
        return <span className="ainative-block">{latexSegments}</span>;
      }
    ...
    if (inPTag) return <span className="ainative-block">{contents}</span>;
    return <p>{contents}</p>;
  }
  ...
  if (t.type === 'link') {
    return (
      <a
        ...
        className="ainative-underline ainative-cursor-pointer hover:ainative-brightness-90 ainative-transition-all ainative-duration-200 ainative-text-ainative-fg-2">
  ...
  // default
  return (
    <div className="ainative-bg-orange-50 ainative-rounded-sm ainative-overflow-hidden ainative-p-2">
			<span className="ainative-text-sm ainative-text-orange-500">Unknown token rendered...</span>
		</div>);
