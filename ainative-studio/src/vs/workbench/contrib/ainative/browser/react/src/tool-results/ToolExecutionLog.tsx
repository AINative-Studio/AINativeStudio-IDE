			...
			{isExpanded && (
				<div className="log-body">
					...
					<div className="log-filters">
						...
						<button
							className={`filter-btn status-success ${filterStatus === 'success' ? 'active' : ''}`}
						...
						<button
							className={`filter-btn status-error ${filterStatus === 'error' ? 'active' : ''}`}
						...
						<button
							className={`filter-btn status-running ${filterStatus === 'running' ? 'active' : ''}`}
...
const LogEntry: React.FC<{ log: ToolLogEntry }> = ({ log }) => {
	...
	return (
		<div className={`log-entry status-${log.status}`}>
			<div className="log-entry-header" onClick={() => setIsExpanded(!isExpanded)}>
				<div className="flex items-center gap-2">
				...
				<div className="log-entry-body">
					...
					<div className="log-metadata">
						<div className="metadata-row">
							...
							<span className="metadata-value">{log.status}</span>
						</div>
						<div className="metadata-row">
							<span className="metadata-key">Tool:</span>
							<span className="metadata-value">{log.toolName}</span>
						</div>
						{log.operation && (
							<div className="metadata-row">
								<span className="metadata-key">Operation:</span>
								<span className="metadata-value">{log.operation}</span>
							</div>
						)}
						<div className="metadata-row">
							<span className="metadata-key">Timestamp:</span>
							<span className="metadata-value">{log.timestamp.toISOString()}</span>
						</div>
						{log.duration !== undefined && (
							<div className="metadata-row">
